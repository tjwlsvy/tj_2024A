...
// 4. 수정함수 실행조건 : 1. (누구를 =인덱스)수정버튼 클릭할때
function _update(index){ 
    ...
    let ucontent = prompt('수정할제목');
    
    // 수정된 정보로 구성 : 제목과 내용만 수정변수로 구성하고 나머지는 기존 데이터 사용
    let uboard = `${utitle},${ucontent},${board[2]},${board[3]},${board[4]}`;
    //
    boardList[index] = uboard;
    //화면 새로고침(재 출력; : 데이터ㄱ 변화가 있기 때문에)
    _allRead();
    _read(index);

}
// 5. 삭제함수 수정조건: 1. (누구를)삭제버튼 클릭할때
function _delete(index){ 
    ...
    //화면 새로고침(재 출력; : 데이터ㄱ 변화가 있기 때문에)
    _allRead();
    document.querySelector('#vuewPage').innerHTML ='';
...
function _pwConfirm( index ){
    ...
    // 2. 해당 인덱스의 비밀번호와 입력받은 비밀번호가 다르면  함수를 종료한다.
    if(confirmPw != boardList[index].split(',') [2]){
       alert('패스워드가 다릅니다.')
        return false;     // 패스워드 일치 실패 뜻 , 함수종료, 이하 아래코드는 실행이 안됨
