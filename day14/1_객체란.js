...
- 객체
  ...
  - 프로그래밍 객체
    ...
    - 인기가 많은이유 : 생활속 모든 정의된(성질/행위)를 디지털화 할수있다. 코드화
  ...
  - 예시 
    자동차 
        ...
        - 성질 : 색상 , 타이어 , 의자 , 등등
        - 행동 : 전진 , 후진 , 파킹 ,잠금 등등
        나의 k3와 친구 k3는 서로 고유한 성질/행동 가질수 있다. 그러므로 다른 객체다.
...
console.log(객체변수.직업);
//console.log(객체변수.address-city);       // 속성명에 특수문자 포함될 경우 오류발생
console.log(객체변수["address-city"]);      // 속성명에 특수문자 있을경우 , 접근 연산자 말고 [ ] 연산자를 사용하자.

// 3, 속성내 데이터 수정,
객체변수.age = 50;     // 만일 있는 속성명일 경우 새로운 데이터 대입
객체변수.money = 300; // 만일 없는 속성명에 데이터를 대입할경우 새로운 속성명과 데이터가 추가
console.log(객체변수);
...
delete 객체변수.money;
console.log(객체변수);
객체변수.직업 = undefined;  // 속성은 있고 속성내 데이터가 없을때 undefined
console.log(객체변수);
