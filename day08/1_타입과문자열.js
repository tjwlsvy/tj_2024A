...
0. 키워드 : JS회사에서 미리 만든 의미/기능 담김 단어
...
console.log('유재석' + 나이)                // 문자열+변수 : 유재석40
...
let ageHTML1 = "<ol> <li>유재석</li><li>강호동</li> </ol>";
document.write(ageHTML1);

let ageHTML2 = `<ol> <li>유재석</li><li>강호동</li> </ol>`;
document.write(ageHTML2);

// 5.
let 차량번호 = '250어7142'
console,log(차량번호[1]);       // 5
console.log(차랑변호.length);   // 8
let 잘라낸차량번호 = 차량번호.split('어');           // 특정 문자 기준으로 분리해서 배열반환
console.log(차량번호);

// 예1> 2명의 사원정보를 이름 입력받아 HTML(table)형식 출력하시오.

    let 사원1 = prompt('사원1 사원명 : '); // 사원 이름을 입력받기 2 변수에 저장
    let 사원2 = prompt('사원2 사원명 : '); 
    // HTML 형식의 문자열을 저장하는배열
        // 1. HTML 형식의 문자열을 저장하는 배열
        // 2. `` 빽틱 템플릿 에서 벼수를 호출시 $(변수명)
    let tableHTML = `<table>
                        <tr>
                            <th>번호</th><th>이름</th>
                        </tr>
    ...
    // 2 . HTML 문자열이 저장된 변수를 HTML에 출력하자
