/*
    ...
    데이터 : 문자 , 숫자 , 불리언 ,undefind , [ ] , { } , function , 배열 , 객체 , 함수
    문자 : ' ' , " " , ` `
    숫자 : 1
    불리언 : true , false 
    배열 : [데이터 , 데이터]
    객체 : { 속성명 : 데이터 , 속성명 : 데이터 }
    함수 : 1. function 함수명(){ }, function(){} 3. () => {}
    
    -  객체안에 여러가지 데이터 / 자료 포함시킬수 있다.  (라이브러리 제공할때 편리함)

    - 브라우저 세션과 쿠키
    f12[개발자도구] -> application  -> 1. localStorage 2.sessionStorage

    - JS 실행시점 : http(주소) 이용한 html요청했을때------>html열렸을때(html에서 js를 포함하고 있기때문에)
    - F5(새로고침) : 현재 HTTP(주소) 재용청 => html -> js(메모리 초기화)
        - js에 있는 메모리 영구 저장(외부 이용)
        1. DataBase( DB ) 2. 브라우저( 웹프로그램 ) 3. 자바( 서버 ) 

    브라우저 저장소   ( 보안 취약 )
     1. localStorage    객체    :   브라우저 꺼져도 데이터 유지. , 유저가 직접 삭제 (활용 : 자동 로그인)
     2.sessionStorage   객체    :   프로그램(브라우저) 꺼지면 데이터 사라짐.(활용 : 검색엔진 , 임시데이터 )
        - 속성 
        .setItem('Key , value);     : 세션/쿠키에 key와value 쌍으로 데이터 저장
