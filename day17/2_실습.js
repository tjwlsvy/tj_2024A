...
function 등록함수(){
    ...
    console.log(localStorage.getItem('방문목록'))   // 없으면 : null ,으면 []
    방문목록 = JSON.parse(localStorage.getItem('방문목록'));
    console.log(방문목록);
    // 만약에 기존에 localStorage가 없으면 빈배열 대입.
    if(방문목록 == null){
        방문목록 = []; }

    방문목록.push(방문); console.log(방문목록);
...
function 출력함수(){
    ...
    // 먼저 기존에 localStorage가 있는 데이터 가져오기
    방문목록 = JSON.parse(localStorage.getItem('방문목록'));
    if(방문목록 == null)return;

    // 1.
    let 출력구역 = document.querySelector('#출력구역');       console.log('#출력구역')

    // 2.
    let html = 'o';
        for( let i = 0 ; i < 방문목록.length ; i++){
            html+=`<div>${방문목록[i].내용} / ${방문목록[i].작성일}
                    </div>`
        }
